...
class GameServer {
    ...
    saveFilePath;
    constructor() {
        this.saveFilePath = path_1.default.join(process.cwd(), 'world_save.json');
        this.loadState();
        this.initEntities();
        // 20 TPS Tick loop
    ...
    handlePlaceBuilding(ownerId, data) {
        this.buildings.push({
    ...
    broadcastWorldState() {
        ...
        this.players.forEach(p => {
            if (p.ws && p.ws.readyState === ws_1.WebSocket.OPEN) {
                p.ws.send(packet);
            }
        });
    }
    broadcastChat(author, text, isSystem = false) {
        const packet = JSON.stringify({
            type: 'chat_message',
    ...
    loadState() {
        if (fs_1.default.existsSync(this.saveFilePath)) {
            try {
                const raw = fs_1.default.readFileSync(this.saveFilePath, 'utf8');
    ...
    saveState() {
        try {
            ...
            fs_1.default.writeFileSync(this.saveFilePath, JSON.stringify(data, null, 2));
        }
        catch (err) {
